...
Pass lists of equal length to `deltas`, `omega`, and `rho`. Each list element
is a formula for that change-point's sub-model.

Instead of writing out priors manually, we can use the `space_omega_priors()`
helper to intelligently blanked our time domain with candidate breakpoints:
...
fit3 <- smoothbp(
  ...
  b0      = ~ 1 + (1 | subject),
  b1      = ~ 1,
  deltas  = list(~ 1, ~ 1, ~ 1),   # three possible slope changes
  omega   = list(~ 1, ~ 1, ~ 1),
  rho     = list(~ 1, ~ 1, ~ 1),
  data    = dat,
  priors  = smoothbp_priors(
    # Automatically generate 3 evenly-spaced priors from tau=0 to tau=6
...
data frame. The `effects` argument filters which parameters are shown:

```{r summary}
print(fit1)                                        # all sections (default)
print(fit1, effects = "fixed")                     # population-level only
summary(fit1, effects = "fixed")                   # same, as a data frame
summary(fit1, effects = c("fixed", "ran_pars"))    # fixed + sigma_u, no u[j]
```
...
smoothbp_priors(
  ...
  b1    = prior_normal(0, 5),
  omega = list(
    prior_normal(2, 1, lb = 0, ub = 4),   # change-point 1
    prior_normal(5, 1, lb = 2)             # change-point 2
  ),
  rho   = list(
...
The `lb` and `ub` arguments impose hard bounds. `omega` should always have
`lb = 0`. `rho` should have `lb = 0` (sharpness must be positive).
...
hypothesis(fit1, "delta1_(Intercept) > 0")

# Does the change-point fall before time 4?
hypothesis(fit1, "omega1_(Intercept) < 4")
```
