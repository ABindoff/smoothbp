...
transition that `smoothbp` uses, making it useful to understand where
the two approaches agree, where they differ, and how the modelling
choice affects inference.

Three scenarios are compared below:

1. **Intercept-only** — all parameters are scalar (the baseline case).
   Compared against **brms**.
2. **Covariate on omega** — `omega ~ 1 + treatment`, which exercises the
   HMC-within-Gibbs sampler path for the non-linear change-point
   parameter. Compared against **brms**.
3. **Hard vs smooth change-point** — single change-point, fixed effects
   only. Compared against **mcp** to contrast modelling assumptions and
   show that the two approaches converge when the transition is sharp.
...
true_vals_cov <- data.frame(
  parameter = c("b0_(Intercept)", "b1_(Intercept)", "b2_(Intercept)",
                 "omega_(Intercept)", "omega_treatment",
                 "rho_(Intercept)", "sigma", "sigma_u"),
  true_value = c(b0_true, b1_true, b2_true,
...
A second structural difference: `mcp` does not directly support
random intercepts on the response level (its random effects are
specifically for *varying change-points*). For a clean comparison
we therefore use a **fixed-effects only** dataset here so that both
models are equivalent in structure.
...
| `time_2`      | `b1_(Intercept) + b2_(Intercept)` (post-change slope) |
...
cat(sprintf("n = %d  |  true omega = 3.2  |  true b1 = -0.4  |  true b2 = 1.4\n",
            nrow(dat_mcp_plain)))
```
...
fit_sbp_mcp <- smoothbp(
  ...
  priors  = smoothbp_priors(
    ...
    b1    = prior_normal(0, 2),
    b2    = prior_normal(0, 2),
    omega = prior_normal(3, 2, lb = 0, ub = max(dat_mcp$tau)),
    rho   = prior_normal(3, 2, lb = 0)
  ),
  chains = 4L, iter = 2000L, warmup = 1000L,
...
equivalent to `b1 + b2` in smoothbp.

```{r mcp-compare, eval = have_mcp}
sbp_sum_mcp <- posterior::summarise_draws(fit_sbp_mcp$draws) |>
  dplyr::filter(variable %in% c("b0_(Intercept)", "b1_(Intercept)",
                                 "b2_(Intercept)", "omega_(Intercept)",
                                 "rho_(Intercept)", "sigma"))

mcp_sum <- summary(fit_mcp) |>
  dplyr::filter(name %in% c("cp_1", "int_1", "time_1", "time_2", "sigma_1"))
...
comparison_mcp <- data.frame(
  ...
  mcp_mean           = mcp_sum$mean[match(c("cp_1","time_1","time_2","sigma_1"),
                                          mcp_sum$name)],
  mcp_lo             = mcp_sum$lower[match(c("cp_1","time_1","time_2","sigma_1"),
                                           mcp_sum$name)],
  mcp_hi             = mcp_sum$upper[match(c("cp_1","time_1","time_2","sigma_1"),
                                           mcp_sum$name)]
)

knitr::kable(comparison_mcp, digits = 3,
  caption = "Scenario 3: smoothbp vs mcp on comparable parameters.
             smoothbp post-change slope is b1 + b2.")
```
...
b2_m   <- mean(draws_sbp_mcp[, "b2_(Intercept)"])
om_m   <- mean(draws_sbp_mcp[, "omega_(Intercept)"])
rho_m  <- mean(draws_sbp_mcp[, "rho_(Intercept)"])

sigmoid  <- function(x) 1 / (1 + exp(-x))
d_grid   <- tau_grid - om_m
mu_sbp   <- b0_m + b1_m * d_grid + b2_m * d_grid * sigmoid(d_grid * rho_m)

# mcp posterior mean trajectory
cp_m    <- mcp_sum$mean[mcp_sum$name == "cp_1"]
int_m   <- mcp_sum$mean[mcp_sum$name == "int_1"]
t1_m    <- mcp_sum$mean[mcp_sum$name == "time_1"]
t2_m    <- mcp_sum$mean[mcp_sum$name == "time_2"]
...
sbp_ess_mcp <- posterior::summarise_draws(
  fit_sbp_mcp$draws[,, c("omega_(Intercept)", "b1_(Intercept)",
                          "b2_(Intercept)", "sigma")],
...
# mcp ESS — convert to posterior draws array for ESS computation
# Use posterior package for compatibility with mcp's output format
mcp_draws <- posterior::as_draws_array(fit_mcp)
mcp_ess_draws <- posterior::summarise_draws(
  mcp_draws[, , c("cp_1", "time_1", "sigma_1")],
  ess_bulk = posterior::ess_bulk
)
...
data.frame(
  ...
  sbp_ess_s   = round(sbp_ess_mcp$ess_per_sec[
    match(c("omega_(Intercept)", "b1_(Intercept)", "sigma"),
...
- **Random effects**: mcp supports varying (random) *change-points* by
  group; smoothbp supports random *intercepts* on the response level.
  These address different scientific questions and the packages are
  complementary rather than strict substitutes.
...
- **Flexibility**: mcp supports multiple change-points, non-Gaussian
  families, variance and autoregressive terms, and quadratic/trig
  segments — features smoothbp does not offer. smoothbp supports
  covariates on all five structural parameters (b0, b1, b2, omega, rho)
  in a single model, which is not directly available in mcp.
