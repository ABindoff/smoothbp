...
A second structural difference: `mcp` supports **varying (random) change-points** by group, while `smoothbp` supports both **random intercepts** on the response level AND **random change-points** (via the `omega = ~ (1 | group)` formula syntax). This makes `smoothbp` highly flexible for complex hierarchical designs. For this comparison, we use a fixed-effects dataset to focus on the core change-point estimation.
...
- **Random effects**: `mcp` supports varying (random) *change-points* by group. `smoothbp` supports both random *intercepts* on the response level AND random *change-points*. This dual support allows `smoothbp` to handle complex nested structures (e.g. subjects within groups) where both the baseline and the timing vary.
