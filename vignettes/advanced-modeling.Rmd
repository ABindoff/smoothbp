...
### Leadership Analysis via Temporal Window Matching

Comparing $\omega_{k,\text{NVDA}}$ with $\omega_{k,\text{MSFT}}$ using a shared index $k$ is valid when the omega priors are centred at the same months for all tickers — which they are here. However, for extra robustness, we use **temporal window matching**: for each draw, we find the breakpoint whose $\omega$ falls within a known calendar window (the early 2023 AI rally), and compare its timing across tickers.

```{r leadership-analysis}
# Rally window: months 3-9 = Jan-Jun 2023
rally_window <- c(3, 9)

get_rally_omega <- function(d, tk, window) {
  oms    <- get_omegas(d, tk)
  in_win <- which(oms >= window[1] & oms <= window[2])
  if (length(in_win) == 0) return(NA)
  oms[in_win[1]]
}
...
rally_omegas <- do.call(rbind, lapply(idx_draws, function(i) {
  d <- draws[i, ]
  data.frame(
    draw       = i,
    omega_NVDA = get_rally_omega(d, "NVDA", rally_window),
    omega_MSFT = get_rally_omega(d, "MSFT", rally_window),
    omega_AAPL = get_rally_omega(d, "AAPL", rally_window)
  )
})) |> na.omit()

p_nvda_msft <- mean(rally_omegas$omega_NVDA < rally_omegas$omega_MSFT)
p_nvda_aapl <- mean(rally_omegas$omega_NVDA < rally_omegas$omega_AAPL)
