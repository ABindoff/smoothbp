...
### How `hierarchical = "omega"` works

In `smoothbp`, hierarchical shrinkage is applied to the **timing** ($\omega$) of structural changes. When you specify `hierarchical = "omega"`:

1.  The model looks at your `omega` formula (e.g., `omega = ~ ticker`).
2.  It identifies the **intercept** as the "Market Mean" timing.
3.  It treats all other coefficients (e.g., ticker-specific offsets) as **random effects** centered on zero.
4.  It estimates a shared precision (shrinkage variance) $\sigma_{re\_om}$ that pulls individual timings toward the market mean.
...
# 1. Create K evenly spaced means, trimming the absolute edges
#    (Using K + 2 and dropping the first and last keeps them inside the window)
om_means <- seq(t_min, t_max, length.out = K + 2)[2:(K + 1)]

# 2. Set a standard deviation that allows adjacent priors to loosely overlap
#    (Width of the domain divided by K is a great heuristic)
om_sd <- (t_max - t_min) / K

# 3. Generate the list of priors
omega_priors <- lapply(om_means, function(m) {
  prior_normal(mean = m, sd = om_sd, lb = t_min, ub = t_max)
})

# Or, if you are using a hierarchical formula like ~ (1 | ticker):
omega_priors_hier <- lapply(om_means, function(m) {
  list(
    # The intercept represents the global market mean
    "(Intercept)" = prior_normal(mean = m, sd = om_sd, lb = t_min, ub = t_max)
    # Note: You don't need to specify priors for the ticker deviations 
    # here because `sigma_re_om` automatically handles their shrinkage
  )
})
