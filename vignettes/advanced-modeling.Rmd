...
Comparing $\omega_{k,\text{NVDA}}$ with $\omega_{k,\text{MSFT}}$ using a shared index $k$ is valid when the omega priors are centred at the same months for all tickers. To be extra robust, we identify which breakpoint index represents the "AI Rally" for each ticker by finding the one whose posterior mean is closest to June 2023 (Month 9).

```{r leadership-analysis}
# Target month for the AI Rally (June 2023)
target_month <- 9

# Identify the BP index closest to the target for each ticker
rally_bp_map <- omega_summary |>
  mutate(dist = abs(mean - target_month)) |>
  group_by(ticker) |>
  slice_min(dist, n = 1) |>
  select(ticker, bp) |>
  ungroup()
...
# For each draw, extract the omega for the identified BP for each ticker
get_ticker_rally_omega <- function(d, tk) {
  bp_idx <- rally_bp_map$bp[rally_bp_map$ticker == tk]
  omegas <- get_omegas(d, tk)
  omegas[bp_idx]
}
...
rally_omegas <- data.frame(
  draw = idx_draws,
  NVDA = sapply(idx_draws, function(i) get_ticker_rally_omega(draws[i,], "NVDA")),
  MSFT = sapply(idx_draws, function(i) get_ticker_rally_omega(draws[i,], "MSFT")),
  AAPL = sapply(idx_draws, function(i) get_ticker_rally_omega(draws[i,], "AAPL"))
)
